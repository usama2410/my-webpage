...
function App() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        let latestObj = res.data.data.length !== 0 ? res.data.data.reduce((a, b) => (a.time > b.time ? a : b)) : null;
        
        let dayData = res.data.data.filter((f) =>
        f.time.substr(0, 10) == latestObj?.time.substr(0, 10))
        
        let hourData = res.data.data.filter((f) =>
        f.time.substr(11, 2) == latestObj?.time.substr(11, 2))
  ...
  let formatedDates=[];
  if(showDay){
    dayData.map((f) => f.latency <= items.length ? ( showLatency.push(f.latency) , formatedDates.push(moment(f.time).format("dddd")) ): null );
  }else {
    hourData.map((f) => f.latency <= items.length ? (showLatency.push(f.latency) , formatedDates.push(f.time.substr(11, 8))): null );
  }
